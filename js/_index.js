    ...
    $.getJSON("https://api.wolfx.jp/sc_eew.json?" + Date.now(),//https://api.wolfx.jp/sc_eew.json
        function (json) {
            ...
            if ((currentTimeStamp - eewStartAt) / 1000 <= 60 + arrivetime) {
                ...
                map.disableDragging();
                setInterval(drawwave, 1000, eewLon, eewLat, eewStartAt, "eew");
            }
            else if (!eewcancel) {
...
function icl() //ICL地震预警网
{
    $.getJSON("https://mobile-new.chinaeew.cn/v1/earlywarnings?start_at=&updates=" + Date.now(), //https://mobile-new.chinaeew.cn/v1/earlywarnings?start_at=&updates=
        function (json) {
            ...
            arrivetime = Math.round(distance / 4);
            if ((currentTimeStamp - iclStartAt) / 1000 <= 60 + arrivetime) {
                iclcancel = false;
...
function drawwave(Lon, Lat, StartAt, type) {
    if (type == "eew" && eewcancel || type == "icl" && iclcancel) {
        return;
    }
...
function countdownRun(Lat, Lon, OriTime) {
    if (!_open)
    {
        return;
    }
